...
\begin{equation}
	...
	\label{eq:24}
\end{equation}
Sabemos que $\ket{\psi_i(t)}$ satisfaz equação de Schrödinger 
\begin{equation}
	i\hbar\pdv{t}\ket{\psi_i(t)} = \hat{H}\ket{\psi_i(t)},
	\label{eq:25}
\end{equation}
então podemos derivar a equação \eqref{eq:23} em relação ao tempo $t$ de modo que:
\begin{eqnarray}
	\pdv{t}\hat{\rho}(t) 
...
\end{eqnarray} 
O conjunto das equações \eqref{eq:5.12}, \eqref{eq:5.13} e \eqref{eq:5.14}  são conhecidas na literatura como equações de Maxwell-Bloch. Neste TCC, essa equações são resolvidas numéricamente, conforme resultados apresentados na seção 1 deste Capítulo.
...
($1/\Gamma_{21} \gg T_p$). É possível calcular a variação da área do pulso, conforme o pulso se propaga no meio,  como descrito em [eberly]. Assim, obtemos que 
\begin{equation}
	\pdv{\theta}{z} = -\frac{\alpha'}{2}sin(\theta)
\end{equation}
onde $\alpha'$ é  o coeficiente de absorção e difere da constante $\alpha$, pois $\alpha' = \alpha \pi G(0)$. Sendo $G(0)$ o valor máximo da função de desintonia.Esse equação é conhecida como teorema da área. Por praticidade, resolvemos numericamente essa equação diferencial, usando método de Euler
\begin{equation}
	\theta(z+\Delta z) = \theta(z) - \frac{\alpha'}{2}sin[\theta(z)] \Delta z, 
\end{equation}
conforme algoritmo escrito em linguagem C apresentado no Apêndice "A". 
...
\begin{figure}[H]
	\centering
	\caption{População $\sigma_{22}$ para um sistema de dois níveis (transição 5s1/2 6s3/2 do Rubídio 87) que interage com um pulso de área pequena $\theta = 5,7*10^{-4}\pi$. (a) Gráfico de temperatura para população $\sigma_{22}(z,t)$. (b) Curva de $\sigma_{22}$ para $z=0$. (c) Curva de $\sigma_{22}$ para $z=1$ cm.}
...
\begin{figure}[!ht]
	\centering
	\caption{Coerência atômica $\sigma_{12}(z,t)$ para um sistema de dois níveis (transição 5s1/2 6s3/2 do Rubídio 87) que interage com um pulso de área pequena $\theta = 5,7*10^{-4}\pi$. (a) Gráfico de temperatura para $\sigma_{12}(z,t)$. (b) Curva de $\sigma_{12}$ para $z=0$. (c) Curva de $\sigma_{12}$ para $z=1$ cm.}
	...
	\end{tikzpicture}
\end{figure}
Na figura \ref{fig:0pimaxwellbloch_s12} temos os resultados numéricos para a coerência $\imaginary{(\sigma_{12})}$. A coerência dos primeiro átomos ($z=0$) é representa na figura \ref{fig:0pimaxwellbloch_s12}(a)  e em \ref{fig:0pimaxwellbloch_s12}(b) temos a coerência do últimos átomos ($z=1$ cm). A coerência atômica obedece a taxa de relaxação $\gamma_{12} = \Gamma_{21}/2$. Por isso, é praticamente desprezível em fig. \ref{fig:0pimaxwellbloch_s12}(a) para o tempo representado.

Na figura \ref{fig:0pimaxwellbloch_freq} apresentamos o resultados para $\Omega_0$ no domínio da frequência. Esses resultados foram obtidos a partir de  FFT (Transformada de Fourier rápida) no resultados numéricos obtidos no domínio do tempo. Em Fig.\ref{fig:0pimaxwellbloch_freq}(a) temos o resultado para $z=0$, em Fig.\ref{fig:0pimaxwellbloch_freq}(b) $z = 0,05$ cm e Fig.\ref{fig:0pimaxwellbloch_freq}(c) $z=1,0$ cm. Observamos que ao se propagar pelo meio a frequências próximas ao centro são absorvidas, pois estão em ressonância com o meio(transição 5s1/2-6p1/2).
\begin{figure}[H]
	\centering
	...
	\begin{tikzpicture}[centered]
		...
		\node[left of=figura,rotate=90, yshift=0.31\linewidth] (legendax) {$\Omega_0(z,\omega)$ [Unid. Arbtrária]};
		\node[below of=figura,yshift=-0.344\linewidth,xshift=0.02\linewidth] (legenday) {($\omega-\omega_0$) [THz]};
		\node[below of= figura,yshift=0.40\linewidth,xshift=0.31\linewidth](a){(a)};
		\node[below of= figura,yshift=0.16\linewidth,xshift=0.31\linewidth](b){(b)};
		\node[below of= figura,yshift=-0.08\linewidth,xshift=0.31\linewidth](a){(c)};
	\end{tikzpicture}
...
Para pulsos com áreas ($2\pi$,$4\pi$, $6\pi$, $\dots$, $2n\pi$), de acordo com o teorema da área, é esperado que a área não mude conforme o pulso se propaga no meio. Esse fenômeno recebe o nome de transparência auto induzida. Em Eberly \cite{allen1987optical} é obtido a solução para um pulso com $\theta=2\pi$, onde é considerado o fato que $\dv*{\theta}{z}=0$ para obter a seguinte equação para $\Omega_0(z,t)$:
\begin{equation}
...
Em  G. L. Lamb, Jr~\cite{gl1971analytical,allen1987optical} encostramos resultados analíticos para $\theta = $\{$4\pi$, $6\pi$ e $0pi$\}. Um pulso de área $\theta = 4\pi$ pode ser descrito pela colisão de dois pulsos com velocidades de grupos $v_g$ e largura temporal $T_p$ diferentes. Assim, temos a seguinte expressão analítica para $\Omega_0(z,t)$:
\begin{equation}
...
\begin{eqnarray}
	...
	\tau' = t - \frac{z}{v_g'}.
\end{eqnarray}
Essas velocidade de grupo, assim como para o caso de $\theta=2\pi$, obedece a relação
\begin{eqnarray}
	v_g = \frac{c}{1+\alpha T_r}\\
	v_g' = \frac{c}{1+\alpha T_r'}
\end{eqnarray}


Na figura \ref{fig:4pimaxwellbloch} temos a solução numérica para frequência de Rabi $\Omega_0$ para a propagação de um pulso de área $\theta=4\pi$. Em (A) temos o gráfico de temperatura onde podemos ver o comportamento do pulso ao se propagar pelo meio. Em (b) temos $\Omega_0$ em $z=0$  e em (c) temos o pulso em $z=10$ cm. Podemos ver que o pulso de área $\theta = 4\pi$ se divide em dois pulsos de área $\theta\approx2\pi$, com largura temporal diferentes, assim como a descrição analítica \eqref{eq:4pianalitico}. O segundo pulso se afasta do primeiro conforme se propaga pelo meio. Isso significa que possuem velocidades de grupo $v_g$ diferentes, sendo que o primeiro mantem a velocidade grupo do pulso inicial (sem propagação). 
...
\section{RESULTADOS EXPERIMENTAIS ENCONTRADO NA LITERATURA}

...
\end{eqnarray}
onde $\mathcal{N}=N_{atomos}/V$ é o número de átomos por unidade de volume e $\langle \vb{d}_i\rangle_{atomos}$ é o momento de dipolo médio da amostra de átomos. Vimos no capitulo \ref{cap:capitulo2} que, na representação de matriz densidade, o valor médio de um dado observável obedece a equação \eqref{eq:6}. Aplicando à equação da polarização \eqref{eq:5.2} para um sistema de dois níveis, obtemos:
\begin{eqnarray}
	\vb{P} &=& \mathcal{N}Tr[\hat{\rho} \hat{d}]\\
	\vb{P} &=& \mathcal{N}(d_{12} p_{21} + d_{21} p_{12}).
\end{eqnarray}
O que estabelece  que a polarização está relacionada a coerência atômica. Como estamos trabalhando com campo que se propaga no eixo z, é conveniente redefinirmos  $\rho_{12} \equiv \sigma_{12}e^{i(kz-\omega t)}$. Assim, considerando o fato que $\rho_{12}=\rho_{21}^*$ e usando \eqref{eq:3.46}, temos que:
\begin{eqnarray}
	\frac{1}{2}\mathcal{P}e^{i(kz-wt)} + \frac{1}{2}\mathcal{P}^*e^{-i(kz-wt)}= \mathcal{N}(d_{12} \sigma_{21}\mathcal{P}e^{i(kz-wt)} + d_{21}\sigma_{12}^*e^{-i(kz-wt)}).
\end{eqnarray}
...
\end{eqnarray} 
O conjunto das equações \eqref{eq:5.12}, \eqref{eq:5.13} e \eqref{eq:5.14}  são conhecidas na literatura como equações de Maxwell-Bloch. Estas equações foram resolvidas numericamente, conforme resultados apresentados na seção 1 deste Capítulo.
...
\begin{equation}
	\pdv{\theta}{\tau} = -\frac{\alpha}{2}sin(\theta)
\end{equation}
onde $\alpha$ é  o coeficiente de absorção. Esse equação é conhecida como teorema da área. Por praticidade, integrando numericamente essa equação diferencial, usando método de Euler
\begin{equation}
...
Nesta seção apresentamos resultados numéricos das equações de Maxwell-Bloch. Esses resultados foram calculados usando algoritmo de Runge-Kutta escrito  em linguagem C. Para essa resolução nos presisamos utilizar o esquema apresentado na figura 2.
...
\begin{figure}[!ht]
	...
	\begin{tikzpicture}[node distance = 3cm, auto]
		% Place nodes
		\node [block,red, text=white](omega) {$\Omega_0$};
		\node [block,green, text=white, below of = omega, xshift=-2cm, yshift=-2cm] (s22) {$\sigma_{22}$};
		\node [block,blue, text=white,right of = s22, xshift=2cm, yshift=-0cm]   (s12) {$\sigma_{12}$};
		\node[below of = omega, xshift=0cm, yshift=1cm] (a) {};
		\node[below of = s22, xshift=0cm, yshift=1cm] (b) {};
		\node[below of = s12, xshift=0cm, yshift=1cm] (c) {};
		\node[right of = omega, xshift=2cm, yshift=0cm] (e) {};
		\node[below of = e, xshift=0cm, yshift=-3cm] (d) {};
		...
		% Draw edges
		\path [line] (omega) -- (a);
		\path [draw,-latex] (a) -- (s22);
		\path [line] (a) -- (s12);
		\path [draw,latex-latex] (s22)   -- (s12);
		\path [line] (s22) -- (b);
		\path [line] (s12) -- (c);
		\path [line] (b) -- (c);
		\path [line] (c) -- (d);
...
OS parâmetros da transição 5s1/2 e 6p1/2 do rubídio 87, apresentados na tabela 1. Para a interação aplicamos com um pulso ultracurto que parte da posição inicial do meio, com formato dado pela secante hiperbólica
\begin{equation}
	\Omega(0,\tau) = \frac{1.76}{T_p}\frac{\theta_0}{\pi}sech\qty(\frac{1.76t}{T_p})
\end{equation} 
onde $t_p$ é a largura temporal do pulso e $\theta_0$ é a área. Utilizamos um pulso de $T_p=100$ fentosegundos e consideramos que o pulso está em ressonância com os átomos de Rubídio. Como o tempo de vida da trasição  5s1/2 e 6p1/2 é $T_{21}=27,70$ ns conseguimos garantir que o teorema da área seja respeitado, visto que $T_{21}=1/\Gamma_{21}\gg T_p$.

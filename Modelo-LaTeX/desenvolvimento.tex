...
\begin{figure}[H]
	...
	\centering
	\sageplot[scale=0.7]{plot(cn(t),t,0,0.5),frame=true,figsize = (4,4),axes=false, axes_labels=("tempo(s)", "$C_n^{i}(t)/c_n^{i}(t)$")}
	\label{fig:delay}
...
%	\end{tikzpicture}
%\end{figure}
\section{Solução numérica das equações opticas de bloch para campos contínuos}

Para estudarmos um campo contínuo, vamos utilizar uma interação de campo contínuo produzido por um laser CW com átomos de Rubídio 87, onde estamos interessados em estudar a transição do estado fundamental $5^2 S_{1/2}$ para o estado excitado $5^2 P_{1/2}$. 
...
\begin{figure}[H]
	...
	\label{fig:p22xdelta}
	\sageplot[scale=0.8]{E, figsize=(8,10)}
\end{figure}
%\begin{figure}[H]
%	\centering
%	\caption{População $\sigma_{22}$ em função da dessintonia $\delta$ em um estado estacionário.}
...
\end{eqnarray}
onde $\mathcal{N}=N_\text{átomos}/V$ é o número de átomos por unidade de volume e $\langle \vb{d}_i\rangle_{atomos}$ é o momento de dipolo médio da amostra de átomos. 
...
\end{eqnarray}
Significa que a polarização é proporcional a coerência atômica. Como estamos trabalhando com campo que se propaga no eixo z, é conveniente redefinirmos $\rho_{12} \equiv \sigma_{12}e^{i(kz-\omega t)}$. Assim, considerando o fato que $\rho_{12}=\rho_{21}^*$ e usando \eqref{eq:3.46}, temos que:
\begin{eqnarray}
...
\begin{eqnarray}
	\mathcal{P}= 2\mathcal{N}d_{12} \int_{-\infty}^{\infty} d\delta G(\delta,t) \sigma_{21}(\delta,t).
	\label{eq:polar2}
\end{eqnarray} 
Trabalharemos com átomos frios de forma que $G(\delta) \approx \delta_d(\delta)$ onde $\delta_d$ é a função delta de Dirac. Com isso, a equação \eqref{eq:polar2} recai em \eqref{eq:polar2} e podemos reescrever a equação \eqref{eq:5.1} da seguinte maneira:
\begin{equation}
...
Na figura \ref{fig:teoremaarea} apresentamos os resultado da equação \eqref{eq:teo_area} para diferente valores $theta_0$ entre $0$ e $8\pi$. Para isso, utilizamos o valor genérico para o coeficiente de absorção $\alpha'= 2\pi$. Podemos ver que após o pulso ultracurtos ao ser propagado pela amostra, os valores das áreas tendem para $\theta(z) = \{0, 2\pi, 4\pi, 6\pi~~\text{e}~~8\pi\}$, onde o valor $n$ que acompanha o $\pi$ é par. Para $0<\theta_0<\pi$, a área do pulso diminui até chegar à zero. Enquanto, para $\pi < \theta_0 < 2\pi$ aumenta para $2\pi$. A mesma tendência é observada para as demais valores de $\theta_0$. Na Seção \ref{sec:res_maxwell_bloch} discutimos mais a fundo esse comportamento a partir dos resultados das equações de Maxwell-bloch.    
%\begin{figure}[H]
%	\centering
...
\begin{figure}[H]
	...
	\caption{Resultado da equação \eqref{eq:teo_area} para um pulso ultracurto, conforme ele se propaga em um meio atômico de dois níveis}
	\label{fig:teoremaarea}
	\sageplot[scale=0.6]{A, frame=true, axes=false, axes_labels=["$z$","$\\theta$"]}
\end{figure}

\section{Metodologia}

Pa resolver a equação de Maxwel bloch apara um pulso ultracurto, adotamos como metologia o calculo numérico através dos método de Runge-Kutta, apresentado no capitulo 4 quando resolvemos a equações ópticas para interação o campo continuo. Para isso foi preciso realizar as seguinte condição de contorno:
Dividimo o e



   conforme apresentado   na Para    Nesta seção apresentamos o algoritmo utilizado para resolver as equações de Maxwell-Bloch, antes temos  Esses resultados foram calculados usando algoritmo de Runge-Kutta escrito  em linguagem C apresentado no apêndice B, seguindo o fluxograma mostrado na figura 2.


\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\draw[fill=green!50!blue] (0,1) -- (0,3) -- (2,4) -- (2,0) -- (0,1) ;
		\draw[fill=green!50!blue] (2,0) -- (2,4) -- (3,3.5) -- (3,3.5) -- (3,0.5) -- (2,0);
	
		\foreach \i in {0.5,1.0,...,1.5} {
			\foreach \j in {1,1.3,...,3} {
				\node at (\i,\j) {2};
		    }	
	    }	

% Template para uso no TCC do Departamento de Física de Ji-Paraná
% da Universidade Federal de Rondônia
% Elaborado em 10 de junho de 2017 por Marco Polo Moreno de Souza com base no template book.
% Aperfeiçoado em 07 de março de 2019, com a introdução de macros
% Modificação na estrutura do de oneside para twoside 1 de setembro de 2019
%Resolução de warning.

\documentclass[a4paper,reqno,12pt,oneside]{book}
\small
%\usepackage[alf]{abntex2cite}
\usepackage{tkz-berge}
\usepackage{tkz-graph}
\usepackage{pgfplots}
\RequirePackage{fix-cm}
\usepackage{amsmath}%
\usepackage{amsfonts}%
\usepackage{amssymb}%
%\usepackage{graphicx}%
\usepackage{physics}%
\usepackage{blindtext}
\usepackage{animate}
\usepackage
[ruled, % Caixa em volta, leg. acima, linha após leg.
portuguese] % Para algoritmos em português.
{algorithm2e} % Enfim o pacote.
%Veja o arquivo abaixo antes de adicionar outras bibliotecas
\usepackage[mode=buildnew]{standalone}% requires -shell-escape
%\usepackage[export]{adjustbox}
\usepackage{upgreek}

\input{bibliotecas.tex}

\makeatletter
\newcommand{\showfont}
{%
	(encoding: \f@encoding{},
	family: \f@family{},
	series: \f@series{},
	shape: \f@shape{},
	size: \f@size{},
	baseline: \f@baselineskip{})
	%tfsize: \tf@size{},
	%sfsize: \sf@size{},
	%sssize: \ssf@size{}
}
\makeatother

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Preencha os campos abaixo conforme seu TCC
\newcommand{\Titulo}{Propagação de pulsos de luz em sistemas atômicos}
\newcommand{\Nome}{Eliton Trindade Gomes}
\newcommand{\curso}{Bacharelado} %Digite Licenciatura Plena ou Bacharelado
\newcommand{\graduacao}{Bacharel} %Digite Licenciado ou Bacharel conforme o caso
\newcommand{\orientador}{Dr. Marco Polo Moreno de Souza} %Nome com título. Ex: Dr. Marco Polo Moreno de Souza
\newcommand{\primeiromembro}{Nome do professor da banca} %Nome com título. Ex: Dr. Fulano de Tal
\newcommand{\segundomembro}{Nome do professor da banca} %Nome com título. Ex: MSc. Beltrano
\newcommand{\instituicao}{DEFIJI/CJP/UNIR} %Instituição do segundo membro. Se for interno, digite DEFIJI/CJP/UNIR
\newcommand{\Cidade}{Ji-Paraná, RO}
\newcommand{\Data}{Mês e ano da defesa}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Dados da defesa, a ser preenchido pelo orientador
\newcommand{\dia}{xxx} %Ex: quinze
\newcommand{\mes}{xxx} %Ex: abril
\newcommand{\ano}{xxx} %Ex: 2019
\newcommand{\inicio}{xxx} %Início da defesa. Ex: 10:15 h
\newcommand{\fim}{xxx} %Fim da sessão. Ex: 11:45 h
\newcommand{\arguicao}{xxx} %Duração da arguição da banca em minutos. Ex: 45
\newcommand{\arguicaoextenso}{xxx} %Idem acima, por extenso. Ex: quarenta e cinco
\newcommand{\local}{xxx} %Ex: auditório do Campus da UNIR de Ji-Paraná
\newcommand{\nota}{xxx} %Ex: 8,5
\newcommand{\notaextenso}{xxx} %Ex: oito inteiros e cinco décimos

\input{macros.tex}
\input{estilo.tex}

\pgfplotsset{compat=1.6}

%\renewenvironment{eqnarray}{\begin{equation}\begin{array}{lcl} }{\end{array}\end{equation}}

\setlength{\sagetexindent}{1.25cm}

\begin{document}
	\pagenumbering{Alph}
	\renewcommand{\bibname}{Referências}
	\input{capa}
	\input{contra-capa.tex}
	\input{ata.tex}
	\input{dedicatoria.tex}
	\input{agradecimentos.tex}
	\input{epigrafe.tex}
	\input{resumo.tex}
	\input{introducao.tex}
	\input{desenvolvimento.tex}
	\input{conclusao.tex}
	%Digite as referências abaixo
	\bibliography{referencias.bib}
	
	\bibliographystyle{ieeetr}
	
	
	\input{apendice.tex}
	
\end{document}